namespace Console_Project_Dungeon_Invade
{
    ...
    /// 6. 재시작 구현 - 재시작을 위한, 맵 백업을 따로 만듬
    /// 6.1  재시작 키를 입력받으면 재시작 안내문이 나옴
    /// 6.2 재시작 시, 처치한 몬스터, 수집한 아이템은 제외하고 박스만 초기화 됨
    /// 6.3 박스만 위치 되돌리는 식으로 코드 짜면 될듯? 반복문에 조건문 돌려서 박스 불일치랑 빈칸 불일치, 골인 불일치 돌리면됨
    /// </summary>
